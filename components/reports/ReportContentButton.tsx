...
type ReportTargetType = "review" | "forum_topic" | "forum_comment";
...
const reportReasons: { value: ReportReasonValue; label: string }[] = [
  ...
  },
  {
    ...
    label: "Spam alebo manipulatívny obsah",
  },
  {
    value: "off_topic",
    label: "Obsah nesúvisí s témou",
  },
  {
    value: "other",
    label: "Iný dôvod",
...
function getTargetLabel(targetType: ReportTargetType) {
  if (targetType === "forum_topic") return "témy";
  if (targetType === "forum_comment") return "komentára";
  return "recenzie";
}
...
export function ReportContentButton({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!user) {
      ...
      setMessage("Pre nahlásenie obsahu sa musíš prihlásiť.");
      return;
    }
  ...
  if (!isOpen) {
    return (
      ...
      >
        ...
        {buttonLabel ?? `Nahlásiť ${getTargetLabel(targetType)}`}
      </Button>
    );
  }
      ...
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          <p className="font-semibold text-amber-950">
            Nahlásenie {getTargetLabel(targetType)}
          </p>
          <p className="mt-1 text-sm text-amber-900">
            Nahlásenie slúži na upozornenie moderátora na obsah, ktorý môže byť
            nevhodný, nepravdivý, mimo témy alebo môže obsahovať osobné či
            citlivé údaje.
          </p>
        </div>
