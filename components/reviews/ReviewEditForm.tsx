...
type ReviewEditFormProps = {
  ...
  initialReview: {
    ...
    rating_waiting_time: number | null;
...
export function ReviewEditForm({
  ...
  const [ratings, setRatings] = useState<Ratings>({
    communication: initialReview.rating_communication ?? 5,
    explanation: initialReview.rating_explanation ?? 5,
    waitingTime: initialReview.rating_waiting_time ?? 5,
    organization: initialReview.rating_organization ?? 5,
    approach: initialReview.rating_approach ?? 5,
    professionalism: initialReview.rating_professionalism ?? 5,
    cleanliness: initialReview.rating_cleanliness ?? 5,
    environment: initialReview.rating_environment ?? 5,
    equipment: initialReview.rating_equipment ?? 5,
    accessibility: initialReview.rating_accessibility ?? 5,
    privacy: initialReview.rating_privacy ?? 5,
    recommendation: initialReview.rating_recommendation ?? 5,
  });

  const [visitType, setVisitType] = useState(
    initialReview.visit_type ?? "first_visit"
  );
  const [isAnonymous, setIsAnonymous] = useState(initialReview.is_anonymous);
  const [manualComment, setManualComment] = useState(
    initialReview.generated_summary?.manualComment ?? ""
  );
  ...
  async function handleSave() {
    ...
    if (error) {
      setMessage(`Úpravu sa nepodarilo uložiť: ${error.message}`);
      return;
    }
    ...
    <div className="space-y-6 rounded-2xl border bg-white p-6 shadow-sm">
      ...
      <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Recenzia sa skladá zo štruktúrovaného hodnotenia a voliteľnej vlastnej
          poznámky. Pri uložení sa znovu analyzuje sentiment, kategórie aj
          rizikový obsah.
        </p>
      </div>
      ...
      <div className="space-y-3 rounded-2xl border bg-slate-50 p-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Môžeš doplniť alebo upraviť vlastný text recenzie. Systém vie
            navrhnúť bezpečnejšiu a vecnejšiu formuláciu.
          </p>
        </div>
        ...
        <textarea
          ...
          placeholder="Doplň vlastnú poznámku k skúsenosti..."
      ...
      <div className="space-y-3 rounded-2xl border bg-white p-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Toto je text, ktorý sa uloží po úprave.
          </p>
        </div>
