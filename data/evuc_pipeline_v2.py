...
def build_start_urls(selected_regions: Optional[list[str]] = None) -> list[str]:
    ...
    regions = [r.strip().lower() for r in regions if r.strip().lower() in REGION_NAMES]

    # Dôležité: ideme po kategóriách a v každej kategórii po krajoch.
    # Tak sa pri menšom --max-pages neprehrabeme iba v BSK, ale crawler
    # aspoň začne všetky kraje.
    urls: list[str] = []
...
def is_detail_page_url(url: str) -> bool:
    """Rozlíši detail zariadenia od okresného zoznamu.

    Typický detail:
      /bsk/zdravotnictvo/ambulantne-zdravotnicke-zariadenia/bratislava-i/ambulancia-xyz.html?page_id=...

    Typický zoznam okresu:
      /bsk/zdravotnictvo/ambulantne-zdravotnicke-zariadenia/bratislava-i.html?page_id=...
    """
    parsed = urlparse(url)
    parts = [p for p in parsed.path.split("/") if p]
    ...
    if len(parts) < 5:
        return False
    if parts[1].lower() != "zdravotnictvo":
        return False
    if not parts[-1].lower().endswith(".html"):
        return False

    # Detail má po kategórii ešte adresár okresu a až potom konkrétny .html súbor.
    # Týmto odstránime okresné listing stránky, ktoré predtým znečisťovali CSV.
    return True
...
def is_listing_title(title: str) -> bool:
    ...
    listing_prefixes = (
        "ambulantne zdravotnicke zariadenia, okres",
        "lekarne a vydajne zp, okres",
        "mobilne odberove miesta, okres",
        "ustavne zdravotnicke zariadenia, okres",
        "domaca osetrovatelska starostlivost, okres",
        "kupelna liecba, okres",
        "ambulantna pohotovost",
        "lekarenska pohotovost",
    )
    return any(norm.startswith(prefix) for prefix in listing_prefixes)


def is_probable_facility_detail(lines: list[str], soup: BeautifulSoup, url: str) -> bool:
    # Najväčšia oprava: okresné zoznamy nie sú záznam zariadenia.
    # Obsahujú veľa mien a identifikátorov, preto predtým prešli cez heuristiku.
    ...
    # Na detailoch zvyčajne stačia aspoň 3 silné polia.
    if strong_hits >= 3:
        return True

    # Niektoré detaily môžu mať slabšie HTML, ale page_id + aspoň 2 polia je ešte OK.
...
def scrape_evuc(
    ...
    # Dve fronty:
    # - normal_queue: kategórie, kraje, okresné listingy, stránkovanie
    # - detail_queue: konkrétne zariadenia
    #
    # Vďaka tomu zbierame detaily rýchlejšie, ale každých pár detailov sa
    # vrátime aj k listingom, aby crawler objavil ďalšie kraje/okresy.
    normal_queue: deque[str] = deque(start_urls)
    detail_queue: deque[str] = deque()
    visited: set[str] = set()
    queued: set[str] = set(start_urls)
    rows_by_key: dict[str, dict[str, str]] = {}

    detail_streak = 0
    max_detail_streak = 25
    ...
    try:
        while (normal_queue or detail_queue) and len(visited) < max_pages:
            if detail_queue and (not normal_queue or detail_streak < max_detail_streak):
                current_url = detail_queue.popleft()
                detail_streak += 1
            else:
            ...
            for link in extract_links_from_page(html, current_url):
                ...
                if is_detail_page_url(link):
                    detail_queue.append(link)
                else:
            ...
            if progress:
                progress.set_postfix({
                    "records": len(rows_by_key),
                    "queue": len(normal_queue) + len(detail_queue),
                    "details": len(detail_queue),
                })
                progress.update(1)
            ...
            if len(visited) % save_every == 0:
                ...
                write_progress(
                    ...
                    queued_count=len(normal_queue) + len(detail_queue),
